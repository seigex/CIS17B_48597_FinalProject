...
void Combat_Screen::calculate_attack(){
    ...
    attack_timer->start(4000);
    attack_bar_timer->start(30);
    ...
    hero_action_label->show();
    if(getStandardEnemy() == true){
        if(!enemy_timer->isActive()){
           enemy_timer->start(5000);
           standardAttackTimer->start(3000);
        }
    }
    if(getBossEnemy() == true){
        if(!bossTimer->isActive()){
            bossTimer->start(6000);
            qDebug() <<"supposed to do this";
        }
    }
...
void Combat_Screen::execute_defend(){
    ...
    defend_bar_timer->start(180);
    defend_timer->start(20000);
    set_defend(true);
...
void Combat_Screen::calculate_enemy_attack(){
    ...
    if(get_defend()==true){
        ...
        enemy_action_label->setText("Enemy defended! -0");
    ...
    calculate_hero_damage(get_enemy_attack());
    hero_health_bar->setValue(get_hero_health());
    enemy_action_label->setText("Enemy Used Random Attack");
    enemy_action_label->setStyleSheet("font: 8pt; color: red;");
    enemy_display_timer->start(3000);
...
void Combat_Screen::lose(){
    ...
    WinScreenEndGame *endGame = new WinScreenEndGame();
